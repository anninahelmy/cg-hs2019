...
void Mesh::set(std::vector<vec3> const& new_pts,  std::vector<Face> const& new_faces) {
	vertices_.clear(), vertices_.reserve(new_pts.size());
	for (const auto &pos : new_pts) {
		Vertex v;
		v.position = pos;
		vertices_.push_back(v);
	}

	triangles_.clear(), triangles_.reserve(new_faces.size());
