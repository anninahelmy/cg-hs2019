...
void Solar_viewer::timer()
{
	if (timer_active_) {
		 ...
		 **/
		float curve_speed = norm(ship_path_.tangent(ship_path_param_));
		ship_path_param_ += ship_speed / curve_speed;
...
void Solar_viewer::paint()
{
	...
	else //in ship
	{
		...
		center = vec4(ship_path_(ship_path_param_), 1.0f);
		rotation = mat4::rotate_y(y_angle_) * mat4::rotate_x(5.0f);
		eye = center + mat4(ship_path_frame_.xyzToFrame()) * rotation * vec4(0, 0, -(dist_factor_ * radius), 0);
...
**/
	// render ship
	mat4 t_matrix = mat4(ship_path_frame_.xyzToFrame());
	m_matrix = mat4::translate(ship_path_(ship_path_param_)) * t_matrix * mat4::scale(ship_.radius_);
	mv_matrix = _view * m_matrix;
	mvp_matrix = _projection * mv_matrix;
	phong_shader_.use();
	...
	/**Render the sun's halo here using the "color_shader_"
