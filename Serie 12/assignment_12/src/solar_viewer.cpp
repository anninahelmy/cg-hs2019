...
keyboard(int key, int scancode, int action, int mods)
{
    if (action == GLFW_PRESS || action == GLFW_REPEAT)
    {
        ...
        switch (key)
        {
            ...
            case GLFW_KEY_N:
            {
                use_normal_mapping_ = !use_normal_mapping_;
                break;
            }

            case GLFW_KEY_J:
            {
                use_ambient_light_ = !use_ambient_light_;
                break;
            }

            case GLFW_KEY_K:
            {
                specular_ = std::max(specular_ - 0.05, 0.0);
                break;
            }
            case GLFW_KEY_L:
            {
                specular_ = std::min(specular_ + 0.05, 1.0);
                break;
            }

            case GLFW_KEY_B:
            {
                proper_background_ = !proper_background_;
                break;
            }

            case GLFW_KEY_H:
            {
                do_sun_tricks_ = !do_sun_tricks_;
                break;
            }
...
void Solar_viewer::initialize()
{
    ...
    earth_.normal_.init(GL_TEXTURE4, GL_TEXTURE_2D, GL_LINEAR_MIPMAP_LINEAR, GL_LINEAR, GL_REPEAT);
    ...
    earth_.normal_.loadPNG(TEXTURE_PATH "/earth_normal.png");
...
void Solar_viewer::paint()
{
    ...
    {
        // Determine the rotation needed to take the billboard's initial normal (0, 0, 1)
        // to the vector pointing from the sun to the eye.
        // Note: positive (clockwise) rotation around x moves the normal in the -y direction, not +y.
        // (These are *extrinsic* Euler angles).
        vec3 desiredNormal = normalize(vec3(eye) - vec3(sun_.pos_));
        billboard_x_angle_ = asin(-desiredNormal.y) * (180 / M_PI);                  // angle in [-90, 90]
        billboard_y_angle_ = atan2(desiredNormal.x, desiredNormal.z) * (180 / M_PI); // angle in (-180, 180]
    }
...
void Solar_viewer::draw_scene(mat4& _projection, mat4& _view)
{
    ...
    sun_shader_.set_uniform("do_sun_tricks", do_sun_tricks_);
    ...
    auto draw_planet = [&,this](Planet &planet)
    {
        ...
        phong_shader_.set_uniform("use_ambient_light", use_ambient_light_);
        planet.tex_.bind();
        unit_sphere_.draw();
    };
    ...
    mv_matrix  = _view * m_matrix;
    mvp_matrix = _projection * mv_matrix;
    n_matrix   = inverse(transpose(mat3(mv_matrix)));
    earth_shader_.use();
    ...
    earth_shader_.set_uniform("t", (float)total_time_, true);
    earth_shader_.set_uniform("use_normal_mapping", use_normal_mapping_);
    earth_shader_.set_uniform("use_ambient_light", use_ambient_light_);
    earth_.tex_.bind();
    unit_sphere_.draw();
...
#if 0
    vec3 sun_in_eyespace(_view(0,3), _view(1,3), _view(2,3));
    vec3 desiredNormal = normalize(-sun_in_eyespace);
    vec3 cam_dx = vec3(1,0,0);
    vec3 cam_dy = vec3(0,1,0);
    vec3 bb_dx = normalize(cam_dx - dot(desiredNormal, cam_dx) * desiredNormal);
    vec3 bb_dy = normalize(cam_dy - dot(desiredNormal, cam_dy) * desiredNormal);
    //m_matrix = mat4::rotate_y(billboard_y_angle_) * mat4::rotate_x(billboard_x_angle_)*mat4::scale(sun_.radius_ * 3);
    mat4 m_rot(0.0f);
    m_rot(0,0) = bb_dx[0]; m_rot(0,1) = bb_dy[0];
    m_rot(1,0) = bb_dx[1]; m_rot(1,1) = bb_dy[1];
    m_rot(2,0) = bb_dx[2]; m_rot(2,1) = bb_dy[2];
    m_rot(3,3) = 1.0f;
    mv_matrix = mat4::translate(sun_in_eyespace) * mat4::scale(sun_.radius_ * 3) * m_rot;
#else
    m_matrix = mat4::rotate_y(billboard_y_angle_) * mat4::rotate_x(billboard_x_angle_)*mat4::scale(sun_.radius_ * 3);
    mv_matrix = _view * m_matrix;
#endif
